...
public:
  ...
  Order(vector<string>& line) {
    ...
    case 5:
      description = line[4];
    case 4:
      if (validItemName(line[3]))
        sequenceNumber = line[3];
      else
        throw string("fail item name not understand -->") + line[3] + "<--";

      if (validItemName(line[2]))
      remover = line[2];
      else
        throw string("pass item name not understand -->") + line[2] + "<--";
      slots = line[1];//the number of slots
      if (validItemName(line[1]))
      installer = line[1]; 
      else
        throw string("item name not understand -->") + line[1] + "<--";

      if (validItemName(line[0]) and not line[0].empty())
      name = line[0];
      if (slots.empty())
        slots = "1";
      else
        throw string("unreadable name for task -->") + line[3] + "<--";
      if (slots.empty())
        slots = "1";
    default:
